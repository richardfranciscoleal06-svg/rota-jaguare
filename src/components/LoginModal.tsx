...
import { X, Shield, UserPlus, LogIn, AlertCircle } from 'lucide-react';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      const user = credentials.find(
        (c) => c.id.toLowerCase() === id.trim().toLowerCase() && c.senha === senha
      );
      if (!user) {
        setError('Credencial inválida. Verifique seu ID e senha.');
        return;
      }
      setError('');
      onLogin(user.id, user.isAdmin);
    } else {
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-rota-border">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-md bg-gradient-to-br from-rota-gold to-rota-red flex items-center justify-center">
