...
export default function PromocaoPage() {
  const [selectedId, setSelectedId] = useState(mockMembers[0].id);
  const [link, setLink] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const member = mockMembers.find((m) => m.id === selectedId)!;
  const currentIndex = mockMembers.findIndex((m) => m.id === selectedId);
  const nextPatente = currentIndex > 0 ? mockMembers[currentIndex - 1].patente : null;
